...
namespace ciel {
	...
	namespace common_type_details {
		...
		template<class T>
		struct common_type_helper<T> : common_type_helper<T, T> {};
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		template<class T1, class T2>
			requires (!is_same_v<T1, decay_t<T1>> || !is_same_v<T2, decay_t<T2>>) && requires { typename common_type<decay_t<T1>, decay_t<T2>>::type; }
		...
		template<class T1, class T2>
			requires requires { typename decay_t<common_type_alias<const remove_reference_t<T1>&, const remove_reference_t<T2>&>>; }
		...
		template<class T1, class T2>
		struct common_type_helper<T1, T2> {
			using type = typename common_type_sub_bullet1<T1 ,T2>::type;
		};
	...
	template<class... T>
	struct common_type {
		using type = typename common_type_details::common_type_helper<T...>::type;
	};
