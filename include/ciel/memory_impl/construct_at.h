...
namespace ciel {
	...
	template<class T, class... Args, class = decltype(::new(ciel::declval<void*>()) T(ciel::declval<Args>()...))>
	constexpr T* construct_at(T* p, Args&& ... args) {
		/*
		以下使用了 placement new，它的形参在正常情况下会传入标准分配函数 void* operator new(size_t, void*)，它直接返回未更改的第二实参
